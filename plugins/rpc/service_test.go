...
func Test_Disabled(t *testing.T) {
	s := &Service{}
	ok, err := s.Init(&Config{Enable: false}, plugins.NewContainer(nil), nil)
...
func Test_Enabled(t *testing.T) {
	s := &Service{}
	ok, err := s.Init(&Config{Enable: true, Listen: "tcp://localhost:9008"}, plugins.NewContainer(nil), nil)
...
func Test_Serve_Errors(t *testing.T) {
	s := &Service{}
	ok, err := s.Init(&Config{Enable: true, Listen: "malformed"}, plugins.NewContainer(nil), nil)
	assert.NoError(t, err)
	assert.True(t, ok)
...
func Test_Serve_Client(t *testing.T) {
	s := &Service{}
	ok, err := s.Init(&Config{Enable: true, Listen: "tcp://localhost:9018"}, plugins.NewContainer(nil), nil)
	assert.NoError(t, err)
	assert.True(t, ok)
...
func TestSetEnv(t *testing.T) {
	...
	ok, err := s.Init(&Config{Enable: true, Listen: "tcp://localhost:9018"}, plugins.NewContainer(nil), e)
